...
const AbsensiPage = () => {
    ...
    // Fungsi untuk mendapatkan data absensi user
    useEffect(() => {
        ...
        const fetchUserAttendanceData = () => {
          ...
          fetch(`http://localhost:3002/${employeeId}/attendance-summary?startDate=${formattedDate}&endDate=${formattedDate}`, {
            method: 'GET'
          })
          .then(response => {
            if (!response.ok) {
        ...
        if (employeeId) {
          Promise.all([fetchUserData(), fetchUserAttendanceData()])
            .catch(error => console.error('Error fetching data:', error));
        }
      }, []);
  
    // Fungsi untuk melakukan submit absensi
    const handleSubmit = () => {
        const today = new Date();
      ...
      <div className="absensi-container">
        {userData && (
          <div className="absensi-content">
            ...
            <p className="current-time">Waktu Sekarang: {currentTime.toLocaleTimeString()}</p>
            <select value={arrivalStatus} onChange={(e) => setArrivalStatus(e.target.value)} className="form-control">
            ...
            {userPresenceData && (
                <button onClick={handleSubmit} className="btn-submit" style={{ display: userPresenceData.departureDate == null ? 'block' : 'none' }}>Submit</button>
            )}
